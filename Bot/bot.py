...
# ================= MongoDB =================
MONGO_URI = os.getenv("MONGO_URI") or "mongodb+srv://Sony:Sony123@sony0.soh6m14.mongodb.net/QuickCodes?retryWrites=true&w=majority"
...
async def cmd_start(m: Message):
    ...
    text = (
        "<b>Welcome to Fast OTP Bot!</b>\n"
        "📖 How to use:\n"
        "1️⃣ Recharge\n2️⃣ Select Country\n3️⃣ Buy Account & Receive OTP\n"
...
async def callback_buy_now(cq: CallbackQuery):
    ...
    if stock <=0: return await cq.answer("❌ Stock not available", show_alert=True)
    if balance < price: return await cq.answer("⚠️ Insufficient balance", show_alert=True)
    number_doc = numbers_col.find_one({"country":country_name,"used":False})
    if not number_doc: return await cq.answer("❌ No available numbers", show_alert=True)
...
async def callback_grab_otp(cq: CallbackQuery):
    ...
    if not number_doc: return await cq.answer("❌ Number not found", show_alert=True)
    string_session = number_doc.get("string_session")
    if not string_session: return await cq.answer("❌ String session missing", show_alert=True)
    client = TelegramClient(StringSession(string_session),API_ID,API_HASH)
    await client.connect()
    try:
        otp_code = None
        async for msg in client.iter_messages("777000",limit=10):
            if msg.message and msg.message.isdigit(): otp_code=msg.message; break
        if not otp_code: return await cq.answer("⚠️ No OTP yet. Try later.", show_alert=True)
        await cq.message.edit_text(f"✅ OTP Received!\n📱 {number_doc['number']}\n🔑 OTP: <code>{otp_code}</code>",parse_mode="HTML")
        await bot.send_message("@thedrxnet",f"📦 Purchase Complete!\n👤 User: {cq.from_user.full_name} (@{cq.from_user.username})\n🌍 Country: {number_doc['country']}\n📱 Number: {number_doc['number']}\n💸 Price: ₹{number_doc.get('price','N/A')}")
...
async def handle_adjust_balance(msg:Message,state:FSMContext):
    ...
    try: user_id=int(uid_str.strip()); amount=float(amt_str.strip())
...
async def handle_add_country(msg: Message):
    if not is_admin(msg.from_user.id) or "," not in msg.text: return
...
# Admin add number with FSM
@dp.message(Command("add"))
async def cmd_add_start(msg: Message, state: FSMContext):
    if not is_admin(msg.from_user.id): return await msg.answer("❌ Not authorized")
    countries = list(countries_col.find({}))
    if not countries: return await msg.answer("❌ No countries found")
    kb = InlineKeyboardBuilder()
    for c in countries:
...
# Number FSM continues here similar to previous OTP grabbing code...
